...
export default function History() {
  ...
  const saveToHistoryViaScreenshot = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/history/capture-invoice`, {
        method: "POST",
  ...
  const handleView = (invoice) => {
    if (invoice.image) {
      const imageUrl = `${API_URL}/history/uploads/${invoice.image
        }?t=${Date.now()}`;
  ...
  const handleResetHistory = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/history/reset`, {
        method: "DELETE",
      });
